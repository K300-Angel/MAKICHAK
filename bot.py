...
engine.setProperty('volume', volume+2.0)#cambiar el volumen
rate = engine.getProperty('rate')#para obtener la velocidad actual
engine.setProperty('rate', rate-40)#cambiar la velocidad con que habla
...
trainer.train([ #Comentar este fragmento después de la primera ejecución exitosa del programa
'Como se que sus productos son de confianza',
'Cada producto ofrecido pertenecen a marcas reconocidas'
])
...
trainer.train([ #Comentar este fragmento después de la primera ejecución exitosa del programa
'Lexy, que productos ofrece la empresa',
...
while True: #Acá se ejecuta todo el flujo entrada/salida de conversaciones con el bot, y el aprendizaje
    ...
    if(str(input1_usuario) == "Lexy esa no es la respuesta" and input2_usuario != 0 and cont_ciclo != 2): #Cuando nos encontramos con 
        ...
        input2_usuario = 0
        while str(input1_usuario) == "Lexy esa no es la respuesta":#Al volver del módulo_2 no podemos
        #decirle al bot nuevamente que no es la respuesta, dado que no se ha iniciado una conversación
            input1_usuario = input('ME: ')


    if(str(input1_usuario) == "Lexy ya sé la respuesta"):#Cuando ya sabemos la respuesta
    ...
    #para enseñarle conversaciones al bot por nuestra cuenta
        ...
        while str(input1_usuario) == "Lexy esa no es la respuesta":#Al volver del módulo_3 no podemos
    ...
    elif(str(input1_usuario) != "Lexy esa no es la respuesta" and str(input1_usuario) != "Lexy ya sé la respuesta"):
...
#Designed by LIMA 2020
#Universidad CESAR VALLEJO DE PERU 
